...
import GetAudioQueue from "@queue/getAudio";
...
import getAuthConfig from "@util/misc/getAuthConfig";
import useOrCreateJwtSecret from "@util/misc/useOrCreateJwtSecret";

const serverAuthConfigured = getAuthConfig().serverPassword ? true : false;
...
const apiRequestRouter = (app: Elysia) => {
  return app
    ...
    .use(
      jwt({
        name: "jwt",
        secret: useOrCreateJwtSecret(),
      }),
    )
    .use(cookie())
    .post(
      "/auth",
      async ({ jwt, cookie, setCookie, set, body: { password } }) => {
        if (password === getAuthConfig().serverPassword) {
          setCookie("auth", await jwt.sign({ role: "user" }), {
            httpOnly: true,
            maxAge: 7 * 86400,
          });
          return `Sign in as ${cookie.role}`;
        } else {
    ...
    .guard(
      {
        async beforeHandle({ set, cookie: { auth }, jwt }) {
          const { role } = await jwt.verify(auth);
          if (serverAuthConfigured && !role) {
            return (set.status = "Unauthorized");
          }
        },
      },
